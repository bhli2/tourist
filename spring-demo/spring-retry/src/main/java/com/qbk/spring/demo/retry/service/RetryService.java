package com.qbk.spring.demo.retry.service;
...
public class RetryService {
    ...
    public String get(int code) throws Exception {
        System.out.println("方法被调用,时间："+ LocalTime.now());
        if (code == 0){
            throw new Exception("情况不对头！");
        }
        System.out.println("方法被调用,情况对头了！");
        return "200";
    }

    /**
     * 这个方法用到了@Recover，也就是用注解方式标记当期方法为回调方法，可以看到传参里面写的是 Exception e，
     * 这个是作为回调的接头暗号（重试次数用完了，还是失败，我们抛出这个Exception e通知触发这个回调方法）。
     */
    @Recover
